...
import { MOCK_INSTITUTIONS, REGION_DATA } from '../lib/constants';
...
const RegionDetail = () => {
  ...
  // States
  const [activeSido] = useState(sido || '서울');
  const [activeSigungu, setActiveSigungu] = useState(sigungu || '전체');
  ...
  const filteredInstitutions = useMemo(() => {
    ...
    // 2. Additional Filter (인증센터, 정직한센터, 저장한 센터)
    if (activeFilter === '인증센터') {
      list = list.slice(0, Math.ceil(list.length * 0.8)); // Mock
    } else if (activeFilter === '저장한 센터') {
      list = []; // Mock
    }
    ...
    return list;
  }, [activeSido, activeSigungu, activeSort, activeFilter]);

  const sortOptions = ['기본순', '평점높은순', '리뷰많은순'];

  const handleInstitutionClick = (inst) => {
    navigate(`/institution/${inst.id}`, { state: { institution: inst, type: 'rehab' } });
  };
    ...
    <div className="bg-[#F8FAFC] min-h-screen pb-20 md:pb-0" onClick={() => setIsSortDropdownOpen(false)}>
      ...
      <header className="md:hidden fixed top-0 w-full z-50 bg-slate-50/80 backdrop-blur-md shadow-sm border-b border-slate-100 h-14 flex items-center px-4">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-sm text-slate-900">{activeSido} {activeSigungu}</span>
        </div>
      </header>
      ...
      <main className="pt-20 md:pt-28 container mx-auto max-w-[1280px] px-4 md:px-6">
        ...
        <div className="mb-4 md:mb-6">
          <p className="text-xs md:text-sm text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1">
            <span className="cursor-pointer hover:text-primary" onClick={() => navigate('/')}>방문재활</span>
            <span className="material-symbols-outlined text-[14px]">chevron_right</span>
            <span>{activeSido}</span>
            <span className="material-symbols-outlined text-[14px]">chevron_right</span>
            <span className="text-primary">{activeSigungu}</span>
          </p>
        </div>
        ...
        <div className="mb-6 md:mb-8 space-y-4 md:space-y-6">
          <div className="flex items-center gap-4 overflow-hidden">
            ...
            <div className="flex gap-2.5 md:gap-3 overflow-x-auto no-scrollbar items-center py-1">
              <button 
                onClick={() => {
                  setActiveSigungu('전체');
                  navigate(`/region/${activeSido}/전체`, { replace: true });
                }}
                className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-[11px] md:text-sm font-bold transition-all whitespace-nowrap ${activeSigungu === '전체' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white text-slate-500 border border-slate-100 hover:border-primary/30'}`}
              >
                전체
              </button>
              {currentSidoData.sub.map(sub => (
                <button 
                  key={sub} 
                  onClick={() => {
                    setActiveSigungu(sub);
                    navigate(`/region/${activeSido}/${sub}`, { replace: true });
                  }}
                  className={`px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-[11px] md:text-sm font-bold transition-all whitespace-nowrap ${activeSigungu === sub ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-white text-slate-500 border border-slate-100 hover:border-primary/30'}`}
                >
                  {sub}
                </button>
              ))}
          ...
          <div className="flex items-start gap-2 mb-6 md:mb-8 relative z-30">
            ...
            <div className="relative shrink-0">
              ...
              {isSortDropdownOpen && (
                <div className="absolute top-full left-0 mt-1.5 w-36 md:w-40 bg-white border border-slate-100 rounded-lg shadow-xl z-50 overflow-hidden">
        ...
        {/* Institution List */}
        <div className="grid gap-3 md:gap-6 md:grid-cols-2 lg:grid-cols-2 pb-12">
          {filteredInstitutions.length > 0 ? (
            ...
            <div className="col-span-full py-20 text-center space-y-4">
              <span className="material-symbols-outlined text-5xl md:text-6xl text-slate-200">location_off</span>
              <p className="text-slate-400 font-bold text-sm md:text-base">이 지역에는 아직 등록된 재활 센터가 없습니다.</p>
              <button 
                onClick={() => {navigate(`/region/${activeSido}/전체`, { replace: true }); setActiveSigungu('전체');}} 
